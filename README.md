...
| `mecanumbot_custom_nav2` | Python | `mecanumbot_frontier_explorer_node`, `mecanumbot_map_agreement_node` | Autonomous exploration -- the T1 pass of the Deep3R seeking system. Runs an RRT frontier detector over slam_toolbox's occupancy grid and drives the best frontier through Nav2's `NavigateToPose`, with the server's reconstruction verdict (`MapCloudAgreement`) both feeding the exit criteria and supplying revisit goals. It replaces no part of Nav2: the exploration tuning is a parameter file in `mecanumbot_description`, and this package is only the part that could not be done in YAML. Latches `exploration/finished` when the scan is done. `mecanumbot_map_agreement_node` runs in **both** phases and is the robot's end of the 2D/3D comparison: it turns the server's verdict into a Nav2 keepout mask (the lidar is one horizontal plane, so a table top or a low step is "free" in the 2D map and lethal in the cloud), a revisit list and RViz markers. It only ever *adds* obstacles. |
