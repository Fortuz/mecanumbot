...
class AudioInputHandler(Node):
    def __init__(self) -> None:
        ...
        self.device_scan_timeout_sec = float(self.get_parameter('device_scan_timeout_sec').value)
    ...
    def _resolve_device(self) -> str:
        ...
        self.get_logger().warn('No capture device detected. Falling back to default ALSA input.')
        return 'default'
    ...
    def _capture_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            self.get_logger().info(f'Starting audio capture with command: {" ".join(command)}')
            ...
            try:
                ...
                self.get_logger().error('arecord is not installed or not available on PATH.')
                return
