...
class AccessoryMonitor(Node):
    ...
    def __init__(self, namespace=""):
        ...
        self.timer = self.create_timer(
            self.dt, self.timer_callback, callback_group=self.callback_group
        )
        self.publishing_state_count = 4
        self.idle_state_count = 0
        self.accessory_cmd = AccessMotorCmd()
        self.cmd_publisher = self.create_publisher(
            AccessMotorCmd, "cmd_accessory_pos", 10
        )
            ...
            if self.publishing_state_count == 4:
                ...
                if self.idle_state_count < 5:
                    ...
                    self.get_logger().info(
                        f"Preparing to start loop {self.loop_count + 1}"
                    )
                    self.loop_count += 1
            else:
    ...
    def assert_accessory_cmd(self):
        if self.publishing_state_count == 0:  # Cam up
            ...
            self.get_logger().info(
                f"Publishing accessory command: n_pos={self.accessory_cmd.n_pos:.2f}"
            )
        if self.publishing_state_count == 1:  # Gripper open
            self.accessory_cmd.gl_pos = (
                MECANUMBOT_MAX_GRIPPER_POS + MECANUMBOT_FRONT_GRIPPER_POS
            ) / 2
            self.accessory_cmd.gr_pos = (
                MECANUMBOT_MIN_GRIPPER_POS + MECANUMBOT_FRONT_GRIPPER_POS
            ) / 2
            self.get_logger().info(
                f"Publishing accessory command: gl_pos={self.accessory_cmd.gl_pos:.2f}, gr_pos={self.accessory_cmd.gr_pos:.2f}"
            )
        if self.publishing_state_count == 2:  # Cam down
            self.accessory_cmd.n_pos = (
                MECANUMBOT_MIN_CAM_POS + MECANUMBOT_MAX_CAM_POS
            ) / 2
            self.get_logger().info(
                f"Publishing accessory command: n_pos={self.accessory_cmd.n_pos:.2f}"
            )
