...
Presents exactly the interface `mecanumbot_io_node` presents on the real robot
(`opencr_state`, `scan`, `cmd_vel`, `cmd_accessory_pos`) plus the simulation-only
ground-truth topics, so everything downstream of the board is backend-agnostic.
The Gazebo Sim equivalent is `gz_io_node`.
"""
...
class MecanumbotMujocoIONode(Node):
    def __init__(self, namespace=""):
        ...
        default_model_path = str(
            Path(get_package_share_directory("mecanumbot_sim"))
            / "mujoco"
            / "mecanumbot.xml"
        )
        ...
        self.wheel_separation_x = float(
            self.get_parameter("robot_params.wheel.separation_x").value
        )
        self.wheel_separation_y = float(
        ...
        self.kinematics = MecanumKinematics(
            ...
            wheel_separation_x=self.wheel_separation_x,
            wheel_separation_y=self.wheel_separation_y,
            vel_tick=self.vel_tick,
        )
        ...
        self.scan_body_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "base_scan"
        )
        self.wheel_joint_ids = {
            "bl": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_JOINT, "wheel_backleft_joint"
            ),
            "br": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_JOINT, "wheel_backright_joint"
            ),
            "fl": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_JOINT, "wheel_frontleft_joint"
            ),
            "fr": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_JOINT, "wheel_frontright_joint"
            ),
        }
        ...
        self.wheel_sign = dict(WHEEL_JOINT_SIGN)
        self.accessory_joint_ids = {
            "n": mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, "head_joint"),
            "gl": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_JOINT, "grabber_left_joint"
            ),
            "gr": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_JOINT, "grabber_right_joint"
            ),
        }
        self.accessory_actuator_ids = {
            "n": mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, "head"),
            "gl": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, "grabber_left"
            ),
            "gr": mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, "grabber_right"
            ),
        }
    ...
    def set_accessory_pose_from_ticks(self) -> None:
        for key, joint_id in self.accessory_joint_ids.items():
            ...
            angle = accessory_ticks_to_angle(self.accessory_ticks[key])
    ...
    def load_scenario(self) -> None:
        ...
        for actor in self.scenario.actors:
            body_id = mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_BODY, actor.body_name
            )
            if body_id < 0:
                raise RuntimeError(
                    f"Scenario actor body {actor.body_name} was not found in the MuJoCo model"
                )
    ...
    def apply_controls(self) -> None:
        for key, actuator_id in self.wheel_actuator_ids.items():
            self.data.ctrl[actuator_id] = self.wheel_sign[
                key
            ] * self.kinematics.ticks_to_wheel_rad_s(self.cmd_ticks[key])

        for key, actuator_id in self.accessory_actuator_ids.items():
            self.data.ctrl[actuator_id] = accessory_ticks_to_angle(
                self.accessory_ticks[key]
            )
    ...
    def build_opencr_state(self) -> OpenCRState:
        ...
        for key, joint_id in self.wheel_joint_ids.items():
            ...
            qpos_adr = self.model.jnt_qposadr[joint_id]
            wheel_states[key] = {
                "vel_ticks": self.kinematics.wheel_rad_s_to_ticks(
                    self.wheel_sign[key] * float(self.data.qvel[dof_adr])
                ),
                "pos_ticks": self.kinematics.wheel_rad_s_to_ticks(
                    self.wheel_sign[key] * float(self.data.qpos[qpos_adr])
                ),
            }
        ...
        msg.pos_n = joint_angle_to_ticks(
            self.data.qpos[self.model.jnt_qposadr[self.accessory_joint_ids["n"]]]
        )
        msg.pos_gl = joint_angle_to_ticks(
            self.data.qpos[self.model.jnt_qposadr[self.accessory_joint_ids["gl"]]]
        )
        msg.pos_gr = joint_angle_to_ticks(
            self.data.qpos[self.model.jnt_qposadr[self.accessory_joint_ids["gr"]]]
        )
    ...
    def publish_scan(self) -> None:
        if self.scan_body_id < 0 or self.scan_rate_hz <= 0.0:
            return
        if self.last_scan_time >= 0.0 and (self.data.time - self.last_scan_time) < (
            1.0 / self.scan_rate_hz
        ):
            return
        ...
        scan.angle_increment = (self.scan_angle_max - self.scan_angle_min) / float(
            max(self.scan_count - 1, 1)
        )
