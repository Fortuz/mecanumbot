...
class MecanumbotSimBehaviorEvaluatorNode(Node):
    def __init__(self):
        ...
        self.human_stop_distance = float(
            self.get_parameter("human_stop_distance").value
        )
        self.wall_stop_distance = float(self.get_parameter("wall_stop_distance").value)
        self.follow_min_distance = float(
            self.get_parameter("follow_min_distance").value
        )
        self.follow_max_distance = float(
            self.get_parameter("follow_max_distance").value
        )
    ...
    def build_evaluation(self) -> SimBehaviorEvaluation:
        ...
        msg.unsafe_wrong_target_motion = msg.robot_moving and (
            ...
            or detection.status
            in ("false_wall_lock", "wrong_human_lock", "subject_error_high")
        )
        ...
        too_close_to_subject = (
            subject is not None and msg.subject_distance < self.follow_min_distance
        )
        too_far_from_subject = (
            subject is not None and msg.subject_distance > self.follow_max_distance
        )

        msg.unsafe_motion = msg.robot_moving and (
        ...
        if msg.unsafe_motion and msg.unsafe_stale_detection_motion:
            ...
            msg.reason = (
                "Robot is moving while raw detector output is missing or stale."
            )
    ...
    def find_nearest_kind(
        self, actors: list[SimActor], kind: str
    ) -> tuple[SimActor | None, float]:
