...
class H264CameraPublisherNode(Node):
    def __init__(self) -> None:
        ...
        self.hardware_encoder = str(
            self.get_parameter("hardware_encoder").value
        ).lower()
        self.custom_pipeline = str(self.get_parameter("custom_pipeline").value)
        self.queue_size = int(self.get_parameter("queue_size").value)

        self.publisher = self.create_publisher(
            CompressedImage, self.topic_name, self.queue_size
        )
    ...
    def _detect_hardware_encoder(self) -> str:
        try:
            result = subprocess.run(
                ["gst-inspect-1.0", "nvv4l2h264enc"], capture_output=True, timeout=2
            )
        ...
        try:
            result = subprocess.run(
                ["gst-inspect-1.0", "vaapih264enc"], capture_output=True, timeout=2
            )
            if result.returncode == 0:
                return "vaapi"
        except Exception:
        ...
        try:
            result = subprocess.run(
                ["gst-inspect-1.0", "omxh264enc"], capture_output=True, timeout=2
            )
            if result.returncode == 0:
                return "omx"
        except Exception:
    ...
    def _build_gstreamer_pipeline(self) -> str:
        ...
        device_str = (
            self.device if not self.device.isdigit() else f"/dev/video{self.device}"
        )
    ...
    def _start_gstreamer_pipeline(self) -> None:
        ...
        try:
            ...
            self.reader_thread = threading.Thread(
                target=self._reader_worker, daemon=True
            )
