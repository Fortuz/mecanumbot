...
class CompressedCameraPublisherNode(Node):
    def __init__(self) -> None:
        ...
        self.csi_gstreamer_pipeline = str(
            self.get_parameter("csi_gstreamer_pipeline").value
        )
    ...
    def _open_usb_capture(self, device: str) -> cv2.VideoCapture | None:
    ...
    def _capture_worker(self) -> None:
        while not self.stop_event.is_set():
            ...
            if not success or frame is None:
                self.get_logger().warn(
                    "Failed to read frame in capture thread. Reopening camera."
                )
    ...
    def _timer_callback(self) -> None:
        if not self.use_capture_thread:
            ...
            with self.capture_lock:
                if (
                    ...
                    or self.latest_seq == self.last_published_seq
                ):
                    return
        ...
        success, encoded = cv2.imencode(
            f".{self.format_string}", frame, self.encode_params
        )
        if not success:
