...
| `launch/mapping.launch.py`                    | Starts `slam_toolbox` (`async_slam_toolbox_node`) with `mecanumbot_description/param/mecanumbot_slam_mapping.yaml`, plus `mecanumbot_core`'s `mecanumbot_scan_grid_node`: slam_toolbox reads `/mecanumbot/scan_grid`, the LD08 scan on a fixed angular grid, not the driver's own scan (see `mecanumbot_core/README.md`). |
...
- SSID to map mapping, done by `nav2.launch.py` (and so by the base launch under `use_nav2`; the copy in `launch_external.launch.py` is commented out): `MecanumNet` -> `AI_dept`, `MecanumetoNet` -> `LED_exp` (with keepout mask). Anything else falls back to `AI_dept`.
