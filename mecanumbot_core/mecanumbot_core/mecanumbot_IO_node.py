...
class Mecanumbot_IO_Node(Node):
    ...
    def __init__(self, namespace=""):
        ...
        self.full_packet_size = (
            len(self.magic) + self.seq_size + self.payload_size + self.crc_size
        )

        self.vel_tick = (
            self.get_parameter("robot_params.wheel.vel_tick").value / 60
        )  # rot/min to rot/s, in 1 int diff
        self.max_wheel_speed = self.get_parameter(
            "plausibility_params.max_wheel_speed"
        ).value
        ...
        self.max_float_abs = self.get_parameter(
            "plausibility_params.max_float_abs"
        ).value
        self.neck_default = self.get_parameter(
            "robot_params.accessory.neck_default"
        ).value
        ...
        self.get_logger().info(
            f"Plausibility: max_speed={self.max_wheel_speed}, pos_range=[{self.min_pos},{self.max_pos}], max_float={self.max_float_abs}"
        )
