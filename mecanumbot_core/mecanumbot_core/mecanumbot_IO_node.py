...
class Mecanumbot_IO_Node(Node):
    ...
    def update_opencr_state_in(self):
        ...
        self.get_logger().info(f"Received neck position from OpenCR: {shorts[20]}, gripper left: {shorts[21]}, gripper right: {shorts[22]}")
        self.opencr_state.pos_n = int(self.cmd_outputs['N_pos'])  # Use the commanded neck position instead of the received value
