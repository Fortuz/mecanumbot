...
class MecanumbotSimDetectionEvaluatorNode(Node):
    def __init__(self):
        ...
        self.subject_match_radius = float(self.get_parameter('subject_match_radius').value)
        self.wall_false_positive_radius = float(self.get_parameter('wall_false_positive_radius').value)
        self.require_raw_detection = bool(self.get_parameter('require_raw_detection').value)
    ...
    def build_evaluation(self) -> SimDetectionEvaluation:
        ...
        nearest_actor, nearest_distance = self.find_nearest_actor(actors, self.latest_subject_pose)
        nearest_wall, nearest_wall_distance = self.find_nearest_wall(actors, self.latest_subject_pose)
        ...
        msg.subject_error = float(self.distance_to_actor(self.latest_subject_pose, subject_actor))
        msg.false_wall_lock = (
        ...
        msg.subject_tracking_ok = (
            raw_detection_ok and
            msg.subject_error <= self.subject_match_radius
    ...
    @classmethod
    def find_nearest_actor(cls, actors: list[SimActor], pose: PoseStamped) -> tuple[SimActor | None, float]:
        if not actors:
            return None, float('inf')
        nearest_actor = min(actors, key=lambda actor: cls.distance_to_actor(pose, actor))
        return nearest_actor, cls.distance_to_actor(pose, nearest_actor)

    @classmethod
    def find_nearest_wall(cls, actors: list[SimActor], pose: PoseStamped) -> tuple[SimActor | None, float]:
        wall_actors = [actor for actor in actors if actor.kind == 'wall']
        if not wall_actors:
            return None, float('inf')
        nearest_wall = min(wall_actors, key=lambda actor: cls.distance_to_actor(pose, actor))
